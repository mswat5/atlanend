...
const timeSlots = [
  { id: 'morning', label: 'Morning', icon: Coffee, color: 'bg-yellow-100 text-yellow-800' },
  { id: 'afternoon', label: 'Afternoon', icon: Sun, color: 'bg-orange-100 text-orange-800' },
  { id: 'evening', label: 'Evening', icon: Sunset, color: 'bg-purple-100 text-purple-800' },
  { id: 'night', label: 'Night', icon: Moon, color: 'bg-blue-100 text-blue-800' },
];
  ...
  return (
    <Card
      ...
      className={`mb-3 transition-all duration-200 ${
        isDragging ? 'shadow-lg rotate-2 scale-105 opacity-50' : 'hover:shadow-md'
      }`}
    >
...
function DaySchedule({ day, activities }: DayScheduleProps) {
  const { reorderScheduleActivities, getFilteredActivities, addActivityToSchedule } = useStore();
  const availableActivities = getFilteredActivities().filter(a => a.isSelected);
      ...
      <CardHeader className="p-4 sm:p-6">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center space-x-2 sm:space-x-3">
            <div className={`p-2 rounded-lg ${day === 'saturday' ? 'bg-blue-100 text-blue-600' : 'bg-purple-100 text-purple-600'}`}>
              <Calendar className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <span className="capitalize text-lg sm:text-xl font-bold">{day}</span>
          </CardTitle>
        ...
        <div className="min-h-[200px]">
          {activities.length === 0 ? (
            <div className="text-center py-8 sm:py-12 text-gray-400 animate-fade-in">
              ...
              <p className="text-sm sm:text-base font-medium">No activities scheduled</p>
              <p className="text-xs sm:text-sm mt-1 text-gray-500">Drag activities here or select from available activities</p>
              {availableActivities.length > 0 && (
                <div className="mt-4 sm:mt-6 space-y-2">
                  <p className="text-xs sm:text-sm font-medium text-gray-600">Quick Add:</p>
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <div className="text-center">
        ...
        <p className="text-gray-600 text-sm sm:text-base">Drag activities to reorder, click to edit details</p>
      </div>
