...
export function ActivityBrowser() {
  const { getFilteredActivities, activities, setCurrentView, schedule } = useStore();
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 mt-6">
            ...
            {scheduledCount > 0 && (
              <div className="flex items-center space-x-2 glass-effect bg-blue-50/80 text-blue-800 px-3 sm:px-4 py-2 rounded-xl shadow-sm animate-fade-in">
                ...
                <span className="font-medium text-sm sm:text-base">{scheduledCount} scheduled</span>
              </div>
            )}
            {selectedCount > 0 && (
              <Button
                onClick={() => setCurrentView('schedule')}
                className="gradient-primary hover:shadow-lg button-hover text-sm sm:text-base px-4 sm:px-6"
              >
                Plan Schedule
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          {filteredActivities.length === 0 ? (
            <div className="text-center py-12 sm:py-16 animate-fade-in">
              <div className="text-gray-400 text-base sm:text-lg mb-2">No activities match your search criteria</div>
              <p className="text-gray-500 text-sm sm:text-base">Try adjusting your filters or search terms</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
              {filteredActivities.map((activity) => (
                <ActivityCard key={activity.id} activity={activity} showScheduleActions={true} />
              ))}
            </div>
          )}
        </div>
