...
import { Calendar, Palette, Clock, Target } from "lucide-react";

export function ScheduleView() {
  const { schedule, currentTheme } = useStore();

  const totalActivities = schedule.saturday.length + schedule.sunday.length;
  const totalDuration = [...schedule.saturday, ...schedule.sunday].reduce(
      ...
      <div className="container-responsive py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
          {stats.map((stat) => {
            ...
            return (
              <Card
                key={stat.label}
                className="glass-effect card-hover animate-slide-in"
              >
                <CardContent className="p-4 sm:p-6">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`p-2 sm:p-3 rounded-xl ${stat.bgColor} ${stat.color} shadow-sm`}
                    >
        ...
        <Tabs defaultValue="schedule" className="space-y-6 sm:space-y-8">
          <TabsList className="grid w-full grid-cols-2 max-w-md mx-auto glass-effect shadow-lg">
            <TabsTrigger
              value="schedule"
              className="flex items-center space-x-2 button-hover"
            >
