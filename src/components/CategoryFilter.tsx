...
const categories: { key: ActivityCategory | 'all'; label: string; icon: any; color: string }[] = [
  ...
  { key: 'food', label: 'Food & Dining', icon: UtensilsCrossed, color: 'bg-orange-500' },
...
export function CategoryFilter() {
  const { selectedCategory, setSelectedCategory, getFilteredActivities } = useStore();
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3 justify-center sm:justify-start">
        {categories.map((category) => {
          ...
          const categoryCount = category.key === 'all' 
            ...
            : filteredActivities.filter(a => a.category === category.key).length;
          
          return (
            <Badge
              key={category.key}
              variant={isSelected ? "default" : "outline"}
              className={`cursor-pointer button-hover px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm glass-effect shadow-sm ${
                isSelected 
